...
const register = async (req, res) => {
    try {
        const { nom, email, password, phone, address, profilePicture, selectedPrestations } = req.body;

        // Vérification si l'email existe déjà
        const existingPrestataire = await Prestataire.findOne({ email });
        if (existingPrestataire) {
            return res.status(400).json({ message: "Cet email est déjà utilisé. veuillez vous connecter pour continuer" });
        }
        ...
        if (typeof selectedPrestations === "string") {
            try {
                ...
                return res.status(400).json({ message: "Erreur dans le format de selectedPrestations" });
            }
        } else {
        ...
        for (let prestation of prestationsArray) {
            const prestationExists = await Prestation.findById(prestation.prestationId);
            if (!prestationExists) {
                return res.status(400).json({ message: `Prestation ID ${prestation.prestationId} non trouvée.` });
            }
            for (let sousPrestationId of prestation.selectedSousPrestations) {
                const sousPrestationExists = await SousPrestation.findById(sousPrestationId);
                if (!sousPrestationExists) {
                    return res.status(400).json({ message: `Sous-prestation ID ${sousPrestationId} non trouvée.` });
                }
            }
        }
...
const getAllPrestataires = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Prestataires récupérés avec succès", prestataires });
    } catch (error) {
        res.status(500).json({ message: "Erreur serveur", error: error.message });
    }
};
...
const updatePrestataire = async (req, res) => {
    try {
        ...
        const updatedPrestataire = await Prestataire.findByIdAndUpdate(id, updatedFields, { new: true }).select("-password");
        if (!updatedPrestataire) {
            return res.status(404).json({ message: "Prestataire non trouvé" });
        }

        res.status(200).json({ message: "Prestataire mis à jour avec succès", prestataire: updatedPrestataire });
    } catch (error) {
        res.status(500).json({ message: "Erreur serveur", error: error.message });
    }
};
...
const deletePrestataire = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Prestataire supprimé avec succès et retiré des sous-prestations" });
    } catch (error) {
        res.status(500).json({ message: "Erreur serveur", error: error.message });
    }
};
...
const getMultiplePrestataires = async (req, res) => {
    try {
        ...
        if (!Array.isArray(ids) || ids.length === 0) {
            return res.status(400).json({ message: "Veuillez fournir une liste d'IDs valide." });
        }

        const prestataires = await Prestataire.find({ _id: { $in: ids } }).select("-password");
        res.status(200).json({ message: "Prestataires récupérés avec succès", prestataires });
    } catch (error) {
        console.error("Erreur lors de la récupération des prestataires:", error);
        res.status(500).json({ message: "Erreur serveur", error: error.message });
...
const addRealisation = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Réalisation ajoutée avec succès", prestataire });
    } catch (error) {
        console.error(error);
...
const deleteRealisation = async (req, res) => {
    try {
        ...
        prestataire.realisations = prestataire.realisations.filter(realisation => realisation !== realisationId);
        await prestataire.save();

        res.status(200).json({ message: "Réalisation supprimée avec succès", prestataire });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: "Erreur serveur" });
    }
...
const fileFilter = (req, file, cb) => {
    const allowedTypes = ["image/jpeg", "image/jpg", , "image/png", "image/gif", "video/mp4", "video/mpeg"];
    if (allowedTypes.includes(file.mimetype)) {
        cb(null, true);
    } else {
        cb(new Error("Format de fichier non pris en charge (JPEG, JPG, PNG, GIF, MP4 uniquement)"), false);
    }
};
