...
const register = async (req, res) => {
    try {
        const { nom, prenom, email, password, phone, address, profilePicture } = req.body;
        ...
        if (!passwordRegex.test(password)) {
            return res.status(400).json({
                message: "Le mot de passe doit avooir au moins 6 caractères, une majuscule, minuscule et un caractère spécial."
            });
        }
        ...
        res.status(201).json({ message: "Inscription réussie ! Connectez-vous pour profiter de nos services", client: newClient });
...
const login = async (req, res) => {
    try {
        ...
        // Générer un token 
        const token = jwt.sign({ id: client._id, role: client.role }, process.env.JWT_SECRET, { expiresIn: "1d" });
        res.status(200).json({ message: "Connexion réussie, profitez de nos services!", token, client });
    } catch (error) {
