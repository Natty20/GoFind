...
const createReservation = async (req, res) => {
    try {
        const { clientId, prestataireId, prestations, date, heure, modePaiement, description } = req.body;
        ...
        if (!client || !prestataire) {
            return res.status(404).json({ message: "Client ou Prestataire introuvable." });
        }
        // voir si les prestations et sous-presta choisi existent dans la bdd
        const prestationsData = await Promise.all(prestations.map(async (prestation) => {
            const prestationObj = await Prestation.findById(prestation.prestationId);
            if (!prestationObj) {
                throw new Error("Prestation non trouvée.");
            }
            const sousPrestationsData = await Promise.all(prestation.sousPrestations.map(async (sousPrestationId) => {
                const sousPrestationObj = await SousPrestation.findById(sousPrestationId);
        ...
        const newReservation = new Reservation({
            client: clientId, prestataire: prestataireId,
            prestations: prestationsData, date,
            heure, modePaiement, description
        });
...
const deleteReservation = async (req, res) => {
    try {
        ...
        if (!reservation) {
            return res.status(404).json({ message: "Réservation non trouvée dans la bdd" });
        }
...
const declineReservation = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Réservation déclinée avec succès!", reservation });
    } catch (error) {
        res.status(500).json({ message: "Erreur du serveur", error });
    }
};
