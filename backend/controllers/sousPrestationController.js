...
const getAllSousPrestations = async (req, res) => {
        try {
            ...
            console.error("Erreur lors de la récupération des sous-prestations :", error);
...
const getSousPrestationById = async (req, res) => {
    try {
        const { id } = req.params;
        const sousPrestation = await SousPrestation.findById(id).populate("prestataires");
        ...
        console.error("Erreur lors de la récupération de la sous-prestation :", error);
...
const addSousPrestation = async (req, res) => {
    try {
        ...
        const { nom, title, shortDescription, longDescription, profileImage, backgroundImage } = req.body;
...
const updateSousPrestation = async (req, res) => {
    try {
        ...
        const updatedSousPrestation = await SousPrestation.findByIdAndUpdate(id, {
            nom,
            title,
            shortDescription,
            longDescription,
        ...
        console.error("Erreur lors de la mise à jour de la sous-prestation :", error);
        res.status(500).json({ message: 'Erreur serveur' });
    }
};
...
const deleteSousPrestation = async (req, res) => {
    try {
        ...
        console.error("Erreur lors de la suppression de la sous-prestation :", error);
        res.status(500).json({ message: 'Erreur serveur' });
    }
};
...
const getSousPrestationsByPrestation = async (req, res) => {
    try {
        ...
        const sousPrestations = await SousPrestation.find({ prestation: prestationId });

        if (!sousPrestations.length) {
            return res.status(404).json({ message: "Aucune autre sous-prestation trouvée pour cette prestation" });
        }
